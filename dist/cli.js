...
const patient_search_1 = require("./patient-search/patient-search");
...
if (email !== undefined && explanation !== undefined) {
    ...
    const patientSearch = new patient_search_1.PatientSearch();
    const results = await patientSearch.search(email);
    if (results.length === 0) {
        console.log(`Couldn't find any patient with email address ${email}.`);
    }
    else {
        console.log(`Found ${results.length} patients with email address ${email}:`);
